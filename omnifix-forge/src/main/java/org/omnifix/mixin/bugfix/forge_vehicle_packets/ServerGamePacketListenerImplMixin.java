package org.omnifix.mixin.bugfix.forge_vehicle_packets;
...
/**
 * Fixes Forge vehicle movement handling that floods chunk packets when the player rides across
 * chunk borders (Forge vehicle packet desync / spam).
 */
...
public class ServerGamePacketListenerImplMixin {
    ...
    @Redirect(
            ...
            require = 0)
    private void omnifix$movePlayerUsingPositionRider(
            ServerPlayer player,
            double x,
            double y,
            double z,
        ...
        if (player == this.player) {
            // Use positionRider so the passenger tracks the vehicle without absMoveTo teleports
            // that re-trigger chunk tracking at every packet.
