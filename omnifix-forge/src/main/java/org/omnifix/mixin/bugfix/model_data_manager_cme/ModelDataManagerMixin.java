package org.omnifix.mixin.bugfix.model_data_manager_cme;
...
/**
 * Fix several concurrency issues in the default Forge {@link ModelDataManager}.
 */
@Mixin(ModelDataManager.class)
public abstract class ModelDataManagerMixin {
    @Shadow
    protected abstract void refreshAt(ChunkPos chunk);
     ...
     * because the returned iterator won't be thread-safe otherwise. See AppliedEnergistics/Applied-Energistics-2#7511
     */
    @ModifyArg(
            ...
            index = 1,
            remap = false)
    private Function<ChunkPos, Set<BlockPos>> omnifix$changeTypeOfSetUsed(
            Function<ChunkPos, Set<BlockPos>> mappingFunction) {
        return pos -> ConcurrentHashMap.newKeySet();
    }

    @Redirect(
            method = "getAt(Lnet/minecraft/world/level/ChunkPos;)Ljava/util/Map;",
            at = @At(
                    value = "INVOKE",
                    target = "Lnet/minecraftforge/client/model/data/ModelDataManager;refreshAt(Lnet/minecraft/world/level/ChunkPos;)V"),
            remap = false)
    private void omnifix$onlyRefreshOnMainThread(ModelDataManager instance, ChunkPos pos) {
